 ...
 * V1              06/09/2023       Vikash                                          Modification for the expiration dates calculation
 * V2              07/09/2029       Vikash                                          Modification for the expiration dated calculation based on new algo/flow
 * V3              14/09/2023       Vikash          Joanna                          Modification for the considering Activated Warranty to don't calculate dates, only post to MS
 * V4              14/11/2023       Vikash          Joanna                          Modification for the date issue as per the jira ticket [ES-3107]
 *                                                                                  Added 'moment.js' library to achieve this. File Location will be 'SuiteScripts/moment.mimn.js
 * V5              04/04/2024       Vikash          Joanna                          Modificatiction as per the jira ticket [ES-3445]
 */
...
define(
    [
        ...
        "N/format",
        "./moment.min.js"
    ], function (runtime, search, https, record, format, moment) {

        const getInputData = () => {
            try {
                //get the search data from the script parameters
                let scriptObj = runtime.getCurrentScript();
                let ssId = scriptObj.getParameter('custscript_warranty_data1');
                log.debug('ssId==', ssId);
                if (!ssId) {
                    return [];
                }
        ...
        const map = (context) => {
            try {
                ...
                let scriptObj = runtime.getCurrentScript();
                let fourYearExtendedWarrantyItem = scriptObj.getParameter('custscript_4yr_ext_warr_item1');
                let fiveYearExtendedWarrantyItem = scriptObj.getParameter('custscript_5yr_ext_warr_item1');
                ...
                let soUpdatedWithWarrantydetails = calculateWarrantyExpiration(soObj1, ifDate, fourYearExtendedWarrantyItem, fiveYearExtendedWarrantyItem);
        ...
        const summarize = (summary) => {
            try {
                ...
                if (soIds.length > 0) {
                    globalConfiguration = getGlobalConfiguration('MuleSoft-Warranty');
                    ...
                    if (payloadData.length > 0) {
                        ...
                        if (chunkData.length > 0) {
                            for (var ci in chunkData) {
                                try {
                                    syncSOWarrantyDataToMuleSoft(chunkData[ci], globalConfiguration);
                                } catch (error) {
            ...
            try {
                const customrecord_tnl_global_configurationSearchObj = search.create({
                    ...
                    columns:
                        [
                            ...
                            search.createColumn({ name: "custrecord_tnl_ms_api_url", label: "MULESOFT Warranty ORDER API URL" })
                        ]
                });
                ...
                customrecord_tnl_global_configurationSearchObj.run().each(function (result) {
                    configurationDetails.push({
                        ...
                        app_warranty_order_api_url: result.getValue('custrecord_tnl_ms_api_url')
                    });
                    return true;
                });
        ...
        const calculateWarrantyExpiration = (soObj, ifDate, fourYearExtendedWarrantyItem, fiveYearExtendedWarrantyItem) => {
            try {
                ...
                let items = ['100-0001', '121-0006', '121-0007', '110-0018', '110-0017', '120-0010', '120-0013', '121-0005'];
                //check so item having any extended warranty item
                ...
                for (let wl = 0; wl < warrantyLineCount; wl++) {
                    try {
                        ...
                        if (warrantyStatus == 1) {
                            ...
                            if (warrantyType == 1) {
                                //check sku
                                if (itemSku == '100-0001' || itemSku == '100-0002' || itemSku == '150-0016' || itemSku == '100-0003') {//Trainer item
                                    //regular
                                    ...
                                    || itemSku == '110-0017' || itemSku == '120-0010' || itemSku == '120-0013' || itemSku == '121-0005') {//accessories
                                    //add 1 yr
                ...
                log.error('Error : In Calculation Warranty Expiration', error);
        ...
        const getSOWarrantyPayload = (soId) => {
            try {
                ...
                let billingAddressSubRecord = soObj.getSubrecord({
                    fieldId: 'billingaddress'
                });

                let b_lable = billingAddressSubRecord.getValue('label');
                let b_country = billingAddressSubRecord.getValue('country');
                let b_attention = billingAddressSubRecord.getValue('attention');
                let b_addresse = billingAddressSubRecord.getValue('addressee');
                let b_phone = billingAddressSubRecord.getValue('addrphone');
                let b_addr1 = billingAddressSubRecord.getValue('addr1');
                let b_addr2 = billingAddressSubRecord.getValue('addr2');
                let b_city = billingAddressSubRecord.getValue('city');
                let b_state = billingAddressSubRecord.getValue('state');
                let b_zip = billingAddressSubRecord.getValue('zip');
        ...
        const syncSOWarrantyDataToMuleSoft = (payloadObj, globalConfiguration) => {
            try {
                log.debug('POST OPERATION', 'RUNNING');
                let request = https.post({
                    body: JSON.stringify(payloadObj),
                    url: globalConfiguration[0].app_warranty_order_api_url,
                    headers: {
