...
define(['N/record', 'N/search', 'N/task', 'N/ui/serverWidget', 'N/ui/message', 'N/redirect', 'N/runtime', '../Library/moment.min.js'],
    function(record, search, task, ui, message, redirect, runtime, moment) {
		function onRequest(context) {
			if (context.request.method == 'GET') {
                ...
                if (uplFile) {
                    ...
                    uplFile.folder = runtime.getCurrentScript().getParameter({name:'custscript_tnl_import_folder_id'}); //PROD
                    var nFileId = uplFile.save();
                }

				newImportRec.setValue({fieldId:'custrecord_tnl_bfi_import_file', value: nFileId});
				newImportRec.setValue({fieldId:'custrecord_tnl_bfi_import_type', value: 1});
				...
				try {
					...
					try {
						var mrTask = task.create({
							...
							scriptId: 'customscript_tnl_bulk_fulfill_proc_mr',
							deploymentId: 'customdeploy_tnl_bulk_fulfill_proc_mr_1'
						});
						mrTask.submit();
					} catch(e) {
		...
		function populateGenRepSublist(sublistObj) {
			var repSearch = search.create({
				...
				filters: [
					['isinactive','is','F'],
					'and',
					['custrecord_tnl_bfi_import_type','is',1]
				],
				columns: [
					'id',
		...
		function checkForPendingImports() {
			var repSearch = search.create({
				type:'customrecord_tnl_bulk_imports',
				filters: [['isinactive','is','F'],'and',['custrecord_tnl_bfi_import_status','is',1],'and',['custrecord_tnl_bfi_import_type','is',1]],
				columns: []
